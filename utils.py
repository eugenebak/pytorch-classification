...
def get_classifier(config):
    norm = True
    if (config.DATASET == "cifar10") or (config.DATASET == "svhn"):
    ...
    if config.CLASSIFIER == "resnet18":
        classifier = resnet18(pretrained=False, device=device, norm=norm).to(device)
    if config.CLASSIFIER == "resnet50":
        classifier = resnet50(pretrained=False, device=device, norm=norm).to(device)
    elif config.CLASSIFIER == "wrnet-28-10":
        classifier = wrnet_28_10(pretrained=False, device=device, norm=norm).to(device)
    elif config.CLASSIFIER == "wrnet-34-10":
        classifier = wrnet_34_10(pretrained=False, device=device, norm=norm).to(device)
    elif config.CLASSIFIER == "wrnet-70-16":
        classifier = wrnet_70_16(pretrained=False, device=device, norm=norm).to(device)
